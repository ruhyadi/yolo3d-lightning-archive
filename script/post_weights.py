...
def post_assets(assets, release_id):
    ...
    for asset in assets:
        ...
        url = (
            ...
            + str(release_id)
            + "/assets?name={}".format(filename)
        )
        print("[INFO] Uploading {}".format(filename))
        response = requests.post(
            url, files={"name": open(asset_path, "rb")}, headers=headers
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", type=str, default="v1.0.1", help="tag name")
    parser.add_argument(
        "--name", type=str, default="Release v1.0.1", help="release name"
    )
    parser.add_argument(
        "--description", type=str, default="v1.0.1", help="release description"
    )
    parser.add_argument(
        "--assets",
        type=tuple,
        default=["weights/regressor_001.pt"],
